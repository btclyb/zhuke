  ...
  return (
    <main className={`relative min-h-screen ${theme === 'dark' ? 'bg-black text-white' : 'bg-gray-50 text-gray-900'} flex items-center justify-center px-6 py-20 transition-colors`}>
      ...
      <div className="relative z-10 max-w-2xl w-full text-center space-y-10">
        ...
        {/* Stats Panel */}
        <div className="fixed bottom-6 right-6 bg-white/10 border border-white/20 backdrop-blur-sm rounded-xl p-4 text-xs opacity-80">
          <div className="flex gap-4">
            <div>{L.stats.total}: <span className="font-semibold">{L.stats.totalNum}</span></div>
            <div>{L.stats.hits}: <span className="font-semibold">{L.stats.hitRate}</span></div>
          </div>
        </div>
